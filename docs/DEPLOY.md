...
| 변수 | 설명 |
| --- | --- |
| `ADMIN_PASSWORD` | 최초 `admin` 계정의 비밀번호 — **반드시 바꾸십시오** |
| `ADMIN_RESET` | `1` 로 한 번 켜면 `admin` 비밀번호 초기화 (분실 시에만) |
| `PORT` | 서버 포트 (기본 3000) |
...
- [ ] 운영자 비밀번호를 8자 이상으로, 추측하기 어렵게 정했는가
- [ ] 담당자가 여러 명이면 **운영자 계정**을 각자 발급했는가 (계정을 공유하면 누가 했는지 기록이 남지 않습니다)
